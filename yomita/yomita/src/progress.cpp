    ...
    void ProgressLearner::work(size_t thread_id)
    {
        ...
        for (int loop = 0; loop < max_loop; loop++)
        {
            for (auto game : games[thread_id])
            {
                ...
                for (int i = 0; i < game.ply - 1; i++)
                {
                    ...
                    if (is_main)
                    {
                        ...
                        if (j++ % interval == 0)
                        {
                            std::cout << "now = " << std::setw(5) << std::setprecision(2) << p * 100 << "%"
                                 << " teacher = " << std::setw(5) << std::setprecision(2) << t * 100 << "%" << std::endl;
                        }
                    }
            ...
            if (is_main)
            {
                ...
                std::cout << std::endl << std::setprecision(8) << "rmse = " << rmse << std::endl;
                j = sum_error = 0;
